...
\begin{figure}[htpb]
    ...
    \caption[L2 error plot without \ac{BM}]{L2 error plot for the \ac{FMM} without \ac{BM} formulation. We can see the error spiking as we approach the characteristic frequencies.}
...
\begin{figure}[htpb]
    ...
    \caption[L2 error plot with \ac{BM}]{L2 error plot for the \ac{FMM} with \ac{BM} formulation. We can see that there are no spikes in the error at the characteristic frequencies, demonstrating the effectiveness of the \ac{BM} formulation. However, we can also see that the overall error is slightly higher than without the \ac{BM} formulation, which is expected due to the reasons explained in \ref{sec:effect-burton-miller}.}
