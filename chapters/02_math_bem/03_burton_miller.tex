...
\newglossaryentry{n0}{
    name={$n_0$ or $\hat{n_0}$},
    description={Local unit normal vector at the collocation point $\mathbf{x_0}$, pointing outward from the surface.}
}
...
\newglossaryentry{eta}{
    name={$\eta$},
    description={Complex coupling parameter used to linearly combine the \acs{CBIE} and \acs{HBIE} in the Burton-Miller formulation}
}
\newglossaryentry{A}{
    name={$A$},
    description={Specific admittance used to define the virtual interior acoustic problem when calculating the optimal coupling parameter}
}
\newglossaryentry{lambda}{
    name={$\lambda$},
    description={Regularization parameter used to prevent matrix ill-conditioning in the Burton-Miller formulation at low frequencies, used as $\eta = i k / \left( k^2 + \lambda \right) $}
}
...
\begin{equation} \label{eq:combined-integral-equation}
    \mathbf{\acs{CBIE}} + \eta \mathbf{\acs{HBIE}} = 0
\end{equation}
A unique solution is guaranteed for \eqref{eq:combined-integral-equation} as long as $\eta$ has a non-zero imaginary part. 
The theoretically optimal choice for this  coupling parameter is $\eta = i/k$  in the high frequency limit \cite{chappellchoicecouplingparameter2009}. 
...
\newglossaryentry{rvec}{
    name={$\mathbf{\vec{r}}$},
    description={Distance vector from the collocation point to the field point, calculated as $\mathbf{x} - \mathbf{x_0}$}
}
...
Integrals diverging as $\mathcal{O}(1/r^3)$ are strictly hypersingular, and cannot be evaluated using Cauchy Principal Value theorems. This necessitates the rigorous application of \ac{HFP} integration \cite{martinhypersingularintegralshow1996}. Standard numerical quadratures like the Gauss-Legendre quadrature will fail to converge for such integrals without applying analytical regularization.
...
\newglossaryentry{G0}{
    name={$G_0$},
    description={Fundamental solution to the Laplace equation}
}
