...
\end{align}
This expansion tell us something quite important about the frequency dependence of this kernel. The dynamic kerm that depends on the wavenumber $k$ is only weakly singular with the order of $\mathcal{O}(1/r)$. The remain ing term, which is purel static and real, is the hypersingular term $1/r^3$.
...
The isolated term in the brackets is completely regularized, and its limit as $r \to 0$  evaluates to $\frac{k^2}{2r}$. Since this term is only weakly singular, we can use standard numericl quadratures like the Gaussian quadrature to integrate it over the flat panel without diverging.
