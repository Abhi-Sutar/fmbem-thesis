...
The first normal derivative of the Green's function with respect to normal $n$ atthe field point $\mathbf{x}$ is then given by:
...
\begin{equation}
    \frac{\partial^2 G_k}{\partial n \partial n_0} \bigg|_{\text{on a single flat panel}} =  \frac{1}{4\pi} \left[\frac{1}{r^3}(1 - ikr)e^{ikr} \right]
\end{equation}
This particular simplification means that we don't have to compute the extra terms associated with the normals nd local tangents.
...
\end{align}
This expansion tells us something quite important about thekernel's frequency dependence. The dynamic term that depends on the wavenumber $k$ is only weakly singular, of the order of $\mathcal{O}(1/r)$. The remaining term, which is purely static and real, is the hypersingular term $1/r^3$.
...
In our code we integrate it with a higher order quadrature to ensure accuracy.